...
import { SiteRecord, MainStockRecord, Site, ActionType, ResourceType } from '../models';
import {
  authenticateToken,
  requireSiteAccess,
...
router.get('/', authenticateToken, async (req, res): Promise<void> => {
  try {
    ...
    if (req.user!.role === UserRole.SITE_MANAGER) {
      ...
      if (siteId && typeof siteId === 'string' && mongoose.Types.ObjectId.isValid(siteId)) {
        if (req.assignedSiteIds?.includes(siteId)) {
          where.site_id = new mongoose.Types.ObjectId(siteId);
        } else {
    ...
    } else if (siteId && typeof siteId === 'string' && mongoose.Types.ObjectId.isValid(siteId)) {
    ...
    const records = await SiteRecord.find(where)
      .sort({ createdAt: -1 })
    ...
    res.json(records.map(r => ({
      ...
      siteName: (r.site_id as any)?.name,
      ...
      recordedBy: (r.recordedBy as any)?._id?.toString() || r.recordedBy?.toString() || r.recordedBy,
...
router.get('/my', authenticateToken, async (req, res): Promise<void> => {
  try {
    ...
    const [records, total] = await Promise.all([
      SiteRecord.find(where)
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limitNum)
        .populate('site_id', 'name location')
        .populate('recordedBy', 'name'),
      SiteRecord.countDocuments(where),
    ]);
...
router.get('/dashboard-stats', authenticateToken, async (req, res): Promise<void> => {
  try {
    ...
    const monthlyRecords = await SiteRecord.find({
      company_id,
    ...
    const pendingRecords = await SiteRecord.countDocuments({
      company_id,
      ...(siteObjectIds.length > 0 && { site_id: { $in: siteObjectIds } }),
    ...
    const recentActivity = await SiteRecord.find({
      company_id,
      ...(siteObjectIds.length > 0 && { site_id: { $in: siteObjectIds } }),
    })
      .sort({ createdAt: -1 })
...
router.get('/:id', authenticateToken, async (req, res): Promise<void> => {
  try {
    ...
    const record = await SiteRecord.findOne({
    ...
    res.json({
      ...
      siteName: (record.site_id as any)?.name,
...
router.put(
  ...
  async (req, res): Promise<void> => {
    try {
      ...
      // Get existing record
      const existingRecord = await SiteRecord.findOne({
        _id: new mongoose.Types.ObjectId(idStr),
        company_id,
      });
      ...
      const mainStockRecord = await MainStockRecord.findById(existingRecord.mainStockEntryId);
      ...
      const updatedRecord = await SiteRecord.findByIdAndUpdate(
        id,
        { $set: updateData },
      ...
      if (!updatedRecord) {
       ...
       const mainStock = await syncSiteRecordToMainStock(idStr);
       ...
       await ActionLogService.logFromRequest(
         ...
         ActionType.UPDATE,
         ResourceType.SITE_RECORD,
         `Updated site record: ${updatedRecord.materialName}`,
         {
       ...
       // Log sync to main stock
       await ActionLogService.logSyncToMainStock(req, updatedRecord._id.toString(), updatedRecord.materialName, updatedRecord.quantityReceived);
...
router.get('/inventory/my', authenticateToken, async (req, res): Promise<void> => {
  try {
    ...
    const inventory = await SiteRecord.aggregate(pipeline);

    // Populate site names
    const siteIdsFromInventory = inventory.map(item => item.site_id.toString());
    const sites = await Site.find({ _id: { $in: siteIdsFromInventory.map((id: string) => new mongoose.Types.ObjectId(id)) } });
...
router.post('/usage', authenticateToken, requireSiteAccess('siteId'), async (req, res): Promise<void> => {
  try {
    ...
    const availableRecords = await SiteRecord.find({
      site_id: new mongoose.Types.ObjectId(siteId),
      materialName,
      company_id,
    });
    ...
    await ActionLogService.logFromRequest(
      ...
      ActionType.CREATE,
      ResourceType.SITE_RECORD,
      `Recorded usage: ${materialName} - ${quantityUsed} units`,
      {
    ...
    // Broadcast update
    broadcastToClients({
      type: 'SITE_RECORD_CREATED' as const,
  ...
  try {
    ...
    if (record.mainStockEntryId) {
      await ActionLogService.logFromRequest(
        ...
        `Deleted main stock record (cascade from site record): ${record.materialName}`,
        {
    ...
    await ActionLogService.logFromRequest(
      ...
      ActionType.DELETE,
      ResourceType.SITE_RECORD,
      `Deleted site record: ${record.materialName}`,
      {
        resourceId: record._id.toString(),
        resourceName: record.materialName,
      }
    ...
    // Delete site record
    await SiteRecord.findByIdAndDelete(id);
